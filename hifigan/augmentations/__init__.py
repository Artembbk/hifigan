...
import hifigan.augmentations.spectrogram_augmentations
import hifigan.augmentations.wave_augmentations
from hifigan.augmentations.sequential import SequentialAugmentation
...
def from_configs(configs: ConfigParser):
    ...
    if "augmentations" in configs.config and "wave" in configs.config["augmentations"]:
        for aug_dict in configs.config["augmentations"]["wave"]:
            wave_augs.append(
                configs.init_obj(aug_dict, hifigan.augmentations.wave_augmentations)
            )
    ...
    if "augmentations" in configs.config and "spectrogram" in configs.config["augmentations"]:
        for aug_dict in configs.config["augmentations"]["spectrogram"]:
            spec_augs.append(
                configs.init_obj(aug_dict, hifigan.augmentations.spectrogram_augmentations)
            )

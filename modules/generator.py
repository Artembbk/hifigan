    ...
    def __init__(self, channels, kernel_size, dilations):
        ...
        for m, _ in enumerate(dilations):
            for l, _ in enumerate(dilations[m]):
                ...
                self.convs.append(nn.Conv1d(channels, channels, kernel_size, dilation=dilations[m][l], padding=padding))
...
class Generator(nn.Module):
    def __init__(self, n_mels, h_init, ku, kr, dilations):
        ...
        self.init_conv = nn.Conv1d(n_mels, h_init, 7, dilation=1, padding=3)
        self.leaky_relu = nn.LeakyReLU()
        self.layers = []
        in_channels = h_init
        for i, kernel_size in enumerate(ku):
            ...
            self.layers.append(nn.ConvTranspose1d(in_channels, out_channels, kernel_size, 
        ...
        self.out_conv = nn.Conv1d(in_channels, 1, 7, padding=3)
        self.tanh = nn.Tanh()
